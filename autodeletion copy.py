...
def extract_text_from_pdf(pdf_path):
    ...
    with open(pdf_path, 'rb') as f:
        ...
        for i, page in enumerate(reader.pages, 1):
            # Print progress update for each page read
...
def find_similarity(file1, file2, window_size):
    ...
    text1 = extract_text_from_pdf(file1) if file1.endswith('.pdf') else open(file1, 'r', encoding='utf-8').read()
    text2 = extract_text_from_pdf(file2) if file2.endswith('.pdf') else open(file2, 'r', encoding='utf-8').read()
...
def find_related_files(directory, file1, window_size):
    ...
    for filename in os.listdir(directory):
        if filename != file1 and (filename.endswith(".txt") or filename.endswith(".pdf")):
