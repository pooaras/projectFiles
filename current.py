...
class CheckboxTreeview(tk.Frame):
    ...
    def insert_checkbox(self, index, filename, similarity):
        item_id = self.treeview.insert("", index, text=filename, values=(filename, similarity))
        ...
        for item_id in self.checked_items:
            try:
                ...
                if item_values is not None:  # Check if the item exists in the treeview
                    checked_items.append(item_values["text"])
    ...
    def delete_selected_files():
        ...
        for item in checked_items:
            ...
            if selected_file:
                ...
                file_path = os.path.join(directory, selected_filename)
                try:
                    os.remove(file_path)
                except OSError as e:
                    messagebox.showerror("Error", f"Failed to delete {selected_filename}: {e}")
