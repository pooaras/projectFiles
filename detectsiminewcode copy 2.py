...
def popup_window(root, files, file_path):
    ...
    toplevel = tk.Toplevel(root) if root else tk.Tk()
    toplevel.title("File Similarity Checker")
    toplevel.geometry("900x700")

    main_frame = tk.Frame(toplevel)
    ...
    label = tk.Label(toplevel, text=f"{file_path}")
    label.pack(pady=5)

    delete_button = tk.Button(toplevel, text="Delete Selected Files", command=delete_selected_files)
...
class NewFileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        popup_window(None, output, file_path)  # Pass None for root as it's not used in this context
